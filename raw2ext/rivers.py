...
def make_axis(lines, bound):
    """Removes the protuding segments from the straight skeleton."""
    LOGGER.debug("- Create axis for %s", bound[0])
    ...
    if len(merge) > 0: # = 1
        if "MULTILINESTRING" in merge[0][0]:
            sql(f"SELECT ST_AsText((ST_Dump('{merge[0][0]}'::geometry)).geom)")
            merge = sql()
        if len(merge) > 1:
            LOGGER.debug("- - %s axis'", len(merge))
    for line in merge:
        sql(f"""
        ...
        """)
    if len(merge) == 0:
        LOGGER.debug("- Can't create axis for %s", bound[0])

def split_basin_river(lines, river):
    """Removes Arain and Tontury from area rivers, adding more river sections in the process."""
    LOGGER.debug("- Remove lakes from %s", river[0])
    ...
    """)
    rings = sql()
    LOGGER.debug("- - new rivers: %s", len(rings))
    for ring in rings:
        sql(f"""
          INSERT INTO {lines} (id, name, type, style, wkb_geometry)
          VALUES (
            nextval('serial'), 'temporary area river', '/STREAMS-LAKE/tmp-river',
            '%fill: #36868d%', ST_Boundary('{ring[0]}'::geometry)
          )
          RETURNING id
        """)
        LOGGER.debug("- add river %s", sql()[0][0])

    sql(f"""
      UPDATE {lines} SET name = 'split temp river', style = 'n/a' WHERE id = {river[0]}
    """)

def split_river(lines, river):
    ...
    for ring in rings:
        sql(f"""
          INSERT INTO {lines} (id, name, type, wkb_geometry)
          VALUES (nextval('serial'), 'candidate', 'STREAMS', '{ring[0]}'::geometry)
        """)
    sql(f"UPDATE {lines} SET name = 'split candidate' WHERE id = {river[0]}")

def connect_to_level(lines, connect_to, level):
    """Find lakes and rivers connected to level lakes and rivers."""
    LOGGER.info("Handle level %s", level)
    ...
    rings = sql()
    if len(rings) > 0:
        LOGGER.debug("- connect %s rivers", len(rings))
    for ring in rings:
        LOGGER.debug("- - line %s", ring[0])
        vertex = 'start' if ring[1] < ring[2] else 'end'
        idx = 0 if vertex == 'start' else -1
        while True:
            sql(f"""
              SELECT ST_NPoints(wkb_geometry) FROM {lines} WHERE id = {ring[0]}
            """)
            line_length = sql()
            sql(f"""
              SELECT ST_Intersects({geom}, wkb_geometry)
              FROM {lines} WHERE id = {ring[0]}
            """)
            intersects = sql()[0]
            if not intersects[0] or line_length[0][0] == 2:
                break
            LOGGER.debug("- - - reduce from %s points", line_length[0][0])
            sql(f"""
              UPDATE {lines} SET wkb_geometry =
        ...
        sql(f"""
          ...
          DELETE FROM {lines} WHERE id = {ring[0]}
        """)
    ...
    lakes = sql()
    if len(lakes) > 0:
        LOGGER.debug("- connect %s lakes", len(lakes))
    for lake in lakes:
        LOGGER.debug("- - lake %s", lake[0])
        sql(f"""
...
def execute(lines):
    ...
    # Initialize
    sql(f"SELECT count(*) FROM {lines} WHERE type LIKE '%STREAMS%'")
    LOGGER.info("Found %s rivers", sql()[0][0])

    sql(f"""
      UPDATE {lines} SET name = 'candidate'
    ...
    """)
    rows = sql()
    LOGGER.info("Removing Arain & Tontury from area rivers: %s", len(rows))
    for row in rows:
        split_basin_river(f"{lines}", row)
    ...
    """)
    rows = sql()
    LOGGER.info("Thinning area rivers: %s", len(rows))
    for row in rows:
        make_axis(lines, row)
    ...
    """)
    rows = sql()
    LOGGER.info("Removing lakes from rivers: %s", len(rows))
    for row in rows:
        split_river(f"{lines}", row)

    # Shores
    level = 0
    sql(f"SELECT ST_Collect(wkb_geometry) FROM {lines} WHERE type = '0'")
    connect_to = sql()

    # Recurse rivers into rivers
    while len(connect_to) > 0 and connect_to[0][0] is not None:
        connect_to_level(lines, connect_to, level)
        level = level + 1
        sql(f"""
          SELECT ST_Collect(wkb_geometry) FROM (
            SELECT wkb_geometry FROM {lines} WHERE type LIKE 'River/{level-1}/%'
            UNION
            SELECT wkb_geometry FROM {lines} WHERE style = 'Connected/{level-1}'
          )
        """)
        connect_to = sql()

    # Remove temporaries from geo_coast
    sql(f"DELETE FROM {lines} WHERE name = 'temporary area river'")

    sql(f"""
      SELECT count(*) FROM {lines}
      WHERE type LIKE '%STREAMS%' AND NOT ST_IsClosed(wkb_geometry) AND NOT (name = '-')
    """)
    LOGGER.info("Leave %s rivers", sql()[0][0])

def tests(inpre):
    """Test collector."""

    # Initialize
    sql(f"CREATE TEMP SEQUENCE IF NOT EXISTS serial START 500000")

    lines = f"{inpre}_lines"
    for typ in ["Lake/test", "COASTLINE/tmp-lake"]:
        test_unit1(lines, typ)
        test_unit2(lines, typ)
        test_unit3(lines, typ)
    test_sample_area1(lines)
    test_sample_lake_river(lines)
    test_sample_area2(lines)
...
def test_unit1(lines, typ):
    ...
    sql(f"""
      INSERT INTO {lines} (id, name, type, wkb_geometry)
      VALUES (
        nextval('serial'), '1', 'STREAMS',
        'LINESTRING(10.1000 10.0001, 10.1000 10.0999)'::geometry
      ),
      (
        nextval('serial'), '2', 'STREAMS',
        'LINESTRING(10.2000 10.0999, 10.2000 10.0001)'::geometry
      ),
      (
        nextval('serial'), '3a', 'STREAMS',
        'LINESTRING(10.3000 10.0001, 10.3000 10.0999)'::geometry
      ),
      (
        nextval('serial'), '3b', '{typ}3',
        'LINESTRING(
          10.3000 10.1000, 10.3100 10.1100,
          10.3000 10.1200, 10.2900 10.1100, 10.3000 10.1000)'::geometry
      ),
      (
        nextval('serial'), '3c', 'STREAMS',
        'LINESTRING(10.3000 10.1201, 10.3000 10.1999)'::geometry
      ),
      (
        nextval('serial'), '4a', 'STREAMS',
        'LINESTRING(10.4000 10.0001, 10.4000 10.0999)'::geometry
      ),
      (
        nextval('serial'), '4b', '{typ}4',
        'LINESTRING(
          10.4000 10.1000, 10.4100 10.1100,
          10.4000 10.1200, 10.3900 10.1100, 10.4000 10.1000
        )'::geometry
      ),
      (
        nextval('serial'), '4c', 'STREAMS',
        'LINESTRING(10.4000 10.1999, 10.4000 10.1201)'::geometry
      ),
      (
        nextval('serial'), '5a', 'STREAMS',
        'LINESTRING(10.5000 10.0999, 10.5000 10.0001)'::geometry
      ),
      (
        nextval('serial'), '5b', '{typ}5',
        'LINESTRING(
          10.5000 10.1000, 10.5100 10.1100,
          10.5000 10.1200, 10.4900 10.1100, 10.5000 10.1000
        )'::geometry
      ),
      (
        nextval('serial'), '5c', 'STREAMS',
        'LINESTRING(10.5000 10.1201, 10.5000 10.1999)'::geometry
      ),
      (
        nextval('serial'), '6a', 'STREAMS',
        'LINESTRING(10.6000 10.0999, 10.6000 10.0001)'::geometry
      ),
      (
        nextval('serial'), '6b', '{typ}6',
        'LINESTRING(
          10.6000 10.1000, 10.6100 10.1100,
          10.6000 10.1200, 10.5900 10.1100, 10.6000 10.1000
        )'::geometry
      ),
      (
        nextval('serial'), '6c', 'STREAMS',
...
def test_unit2(lines, typ):
    ...
    sql(f"""
        INSERT INTO {lines} (id, name, type, wkb_geometry)
        VALUES (
          nextval('serial'), '7a', 'STREAMS',
          'LINESTRING(10.7000 10.0001, 10.7000 10.0999)'::geometry
        ),
        (
          nextval('serial'), '7b', '{typ}7b',
          'LINESTRING(
            10.7000 10.1000, 10.7100 10.1100,
            10.7000 10.1200, 10.6900 10.1100, 10.7000 10.1000)'::geometry
        ),
        (
          nextval('serial'), '7c', 'STREAMS',
          'LINESTRING(10.7000 10.1201, 10.7000 10.1999)'::geometry
        ),
        (
          nextval('serial'), '7d', '{typ}7d',
          'LINESTRING(
            10.7000 10.2000, 10.7100 10.2100,
            10.7000 10.2200, 10.6900 10.2100, 10.7000 10.2000)'::geometry
        ),
        (
          nextval('serial'), '7e', 'STREAMS',
          'LINESTRING(10.7000 10.2201, 10.7000 10.2999)'::geometry
        ),
        (
          nextval('serial'), '8a', 'STREAMS',
          'LINESTRING(10.8000 10.0001, 10.8000 10.0999)'::geometry
        ),
        (
          nextval('serial'), '8b', '{typ}8b',
          'LINESTRING(
            10.8000 10.1000, 10.8100 10.1100,
            10.8000 10.1200, 10.7900 10.1100, 10.8000 10.1000
          )'::geometry
        ),
        (
          nextval('serial'), '8c', 'STREAMS',
          'LINESTRING(10.8000 10.1201, 10.8000 10.1999)'::geometry
        ),
        (
          nextval('serial'), '8d', '{typ}8d',
          'LINESTRING(
            10.8000 10.2000, 10.8100 10.2100,
            10.8000 10.2200, 10.7900 10.2100, 10.8000 10.2000
          )'::geometry
        ),
        (
          nextval('serial'), '8e', 'STREAMS',
          'LINESTRING(10.8000 10.2999, 10.8000 10.2201)'::geometry
        ),
        (
          nextval('serial'), '9a', 'STREAMS',
          'LINESTRING(10.9000 10.0001, 10.9000 10.0999)'::geometry
        ),
        (
          nextval('serial'), '9b', '{typ}9b',
          'LINESTRING(
            10.9000 10.1000, 10.9100 10.1100,
            10.9000 10.1200, 10.8900 10.1100, 10.9000 10.1000
          )'::geometry
        ),
        (
          nextval('serial'), '9c', 'STREAMS',
          'LINESTRING(10.9000 10.1999, 10.9000 10.1201)'::geometry
        ),
        (
          nextval('serial'), '9d', '{typ}9d',
          'LINESTRING(
            10.9000 10.2000, 10.9100 10.2100,
            10.9000 10.2200, 10.8900 10.2100, 10.9000 10.2000
          )'::geometry
        ),
        (
          nextval('serial'), '9e', 'STREAMS',
...
def test_unit3(lines, typ):
    ...
    sql(f"""
        INSERT INTO {lines} (id, name, type, wkb_geometry)
        VALUES (
          nextval('serial'), '10a', 'STREAMS',
          'LINESTRING(11.0000 10.0999, 11.0000 10.0001)'::geometry
        ),
        (
          nextval('serial'), '10b', '{typ}10',
          'LINESTRING(
            11.0000 10.1000, 11.0100 10.1100,
            11.0000 10.1200, 10.9900 10.1100, 11.0000 10.1000
          )'::geometry
        ),
        (
          nextval('serial'), '10c', 'STREAMS',
          'LINESTRING(10.9800 10.1999, 11.0000 10.1201)'::geometry
        ),
        (
          nextval('serial'), '10d', 'STREAMS',
          'LINESTRING(11.0200 10.1999, 11.0000 10.1201)'::geometry
        ),
        (
          nextval('serial'), '11a', 'STREAMS',
          'LINESTRING(11.1000 10.0999, 11.1000 10.0001)'::geometry
        ),
        (
          nextval('serial'), '11b', '{typ}11',
          'LINESTRING(
            11.1000 10.1000, 11.1100 10.1100,
            11.1000 10.1200, 11.0900 10.1100, 11.1000 10.1000
          )'::geometry
        ),
        (
          nextval('serial'), '11c', 'STREAMS',
          'LINESTRING(11.0800 10.1999, 11.1000 10.1201)'::geometry
        ),
        (
          nextval('serial'), '11d', 'STREAMS',
...
def test_sample_area1(lines):
    ...
    sql(f"""
      INSERT INTO {lines} (id, name, type, style, wkb_geometry)
      VALUES (
        17573, '-', '/STREAMS-LAKES/Melderyn/-', 'fill: #36868d',
        'LINESTRING(
          -15.8033738 41.1343803, -15.8035777 41.1338582, -15.8034282 41.1344368,
          ...
          -15.8033194 41.1343803, -15.8033738 41.1343803
        )'::geometry
      )
    """)
    ...
    execute(lines)
    sql(f"""
      SELECT id FROM {lines} WHERE name = 'candidate'
    """)

    sql(f"""
      SELECT count(*) FROM {lines} WHERE name = 'candidate'
    """)
    assert sql()[0][0] >= 1
    print("> 1 test passed")

def test_sample_lake_river(lines):
    ...
    sql(f"""
      INSERT INTO {lines} (id, name, type, style, wkb_geometry)
      VALUES (
        502241, '-', 'River/0/Mouth:start', '',
        'LINESTRING (
          -15.3383724 40.8395570, -15.339359 40.8375898, -15.340109 40.8363858,
          ...
          -15.351031 40.8045607
        )'::geometry
      ),
      (
        17661, '-', 'Lake', '...;fill: #d4effc;...',
        'LINESTRING (
          -15.3460973 40.8023452, -15.3472528 40.8034318, -15.3485577 40.8051674,
          ...
          -15.3461381 40.8023452, -15.3460973 40.8023452
        )'::geometry
      ),
      (
        17551, 'candidate', '/STREAMS-LAKES/Melderyn/-', '...;fill: none;...',
        'LINESTRING (
          -15.3492374 40.7915079, -15.3505987 40.7907402, -15.3517798 40.7900334,
          ...
          -15.4211031 40.7936870, -15.4228855 40.7916490
        )'::geometry
      )
    """)
    ...
    # Test object
    execute(lines)
    sql(f"SELECT count(*) FROM {lines} WHERE type = 'River/2/Mouth:start'")
    assert sql()[0][0] == 1
    print("> 1 test passed")

def test_sample_area2(lines):
    ...
    sql(f"""
      INSERT INTO {lines} (id, name, type, style, wkb_geometry)
      VALUES (
        17609, 'non-closed area river', '/STREAMS-LAKES/Melderyn/-', 'fill: #36868d;',
        'LINESTRING (
          -16.0417992 40.8827364, -16.0453334 40.8814664, -16.0467063 40.8805492,
    ...
    # Test object
    execute(lines)
    sql(f"SELECT count(*) FROM {lines} WHERE name = 'candidate' AND type = 'STREAMS'")
    assert sql()[0][0] >= 1
